...
STACK_DIR="${ROOT_DIR}/config/stacks/${STACK}"
MANIFEST_FILE="${STACK_DIR}/manifest.json"
WORKFLOWS_DIR="${STACK_DIR}/workflows"

if [[ ! -f "$MANIFEST_FILE" || ! -d "$WORKFLOWS_DIR" ]]; then
  echo "Missing stack config for ${STACK}." >&2
  exit 1
fi
...
if [[ ! -f "$WORKFLOWS_DIR/$WORKFLOW_FILE_NAME" ]]; then
  echo "Workflow not found in ${WORKFLOWS_DIR}: ${WORKFLOW_FILE_NAME}" >&2
  exit 1
fi
